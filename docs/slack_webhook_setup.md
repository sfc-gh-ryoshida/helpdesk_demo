...
[ユーザー] 
    ...
    │ ① メッセージ投稿「スキャナーが動かない」
    ▼
[Slack #ryoshida_demo_helpdesk-request]
    │
    │ ② ワークフロービルダーがトリガー
...
1. Slackで新規チャンネルを作成
   - 名前: `#ryoshida_demo_helpdesk-request`
...
4. 投稿先チャンネル: `#ryoshida_demo_helpdesk-request` を選択
...
2. チャンネル: `#ryoshida_demo_helpdesk-request` を選択
3. 続行
...
```json
{
  ...
  "user_name": "田中太郎",
  "user_id": "U0123456789",
  "channel": "#ryoshida_demo_helpdesk-request",
...
1. `#ryoshida_demo_helpdesk-request` に投稿:
   ```
   テスト投稿です
   ```
...
### Slack側
- [ ] `#ryoshida_demo_helpdesk-request` チャンネル作成

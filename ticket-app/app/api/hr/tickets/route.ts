...
export async function GET(request: Request) {
  ...
  let sql = `SELECT * FROM app.hr_tickets WHERE 1=1`;
  let countSql = `SELECT COUNT(*) as total FROM app.hr_tickets WHERE 1=1`;
  ...
  try {
    ...
    return NextResponse.json(
      { error: "Failed to fetch HR tickets" },
      { status: 500 }
    );
  }
}
